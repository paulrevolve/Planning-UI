...
//   const getCurrentPlan = () => {
//   if (!selectedPlan) return null;
//   // Always find the freshest data from the plans array
//   return (
//     plans.find(
//       (p) => p.plId === selectedPlan.plId && p.projId === selectedPlan.projId
//     ) || selectedPlan
...
//           version: plan.version || 0,
//           versionCode: plan.versionCode || "",
//           source: transformSource(plan.source),
//           // source: `${plan.plType === 'BUD' ? 'B' : plan.plType === 'EAC' ? 'E' : ''}${plan.source || 0}`,
//           finalVersion: false,
//           isCompleted: false,
//           isApproved: false,
//           status: "In Progress",
...
//                   disabled={
//     !getCurrentPlan() ||
//     !getButtonAvailability(getCurrentPlan(), "Create Budget")
//   }
//                   className={`btn1 ${
//                     !selectedPlan ||
//                     !getButtonAvailability(selectedPlan, "Create Budget")
...
//                   disabled={
//     !getCurrentPlan() ||
//     !getButtonAvailability(getCurrentPlan(), "Create Blank Budget")
//   }
//                   className={`btn1 ${
//                     !selectedPlan ||
//                     !getButtonAvailability(selectedPlan, "Create Blank Budget")
...
//                   disabled={
//     !getCurrentPlan() ||
//     !getButtonAvailability(getCurrentPlan(), "Create EAC")
//   }
//                   className={`btn1 ${
//                     !selectedPlan ||
//                     !getButtonAvailability(selectedPlan, "Create EAC")
...
//     );
//   }}
//   // CHANGE: Use getCurrentPlan() here
//   disabled={
//     !getCurrentPlan() ||
//     !getButtonAvailability(getCurrentPlan(), "Create Budget")
//   }
  ...
  (
    ...
    const fetchPlans = async () => {
      ...
      try {
        const response = await axios.get(
          // `${backendUrl}/Project/GetProjectPlans/${projectId}`
          `${backendUrl}/Project/GetProjectPlans/${userId}/${role}/${projectId}?status=${status}`
        );
    ...
    const refreshPlans = async (overrideProjId) => {
      ...
      try {
        const response = await axios.get(
          // `${backendUrl}/Project/GetProjectPlans/${projectId}`
          //  ` ${backendUrl}/Project/GetProjectPlans/${effectiveProjId}`
          `${backendUrl}/Project/GetProjectPlans/${userId}/${role}/${effectiveProjId}?status=${status}`
        );
    ...
    const getCheckboxProps = (plan, col, idx) => {
      ...
      if (col === "isApproved")
        return {
          ...
          disabled: !plan.isCompleted || plan.finalVersion,
        };
